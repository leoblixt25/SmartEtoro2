...
def get_current_holdings(db, portfolio_id: int) -> List[Dict]:
    ...
    for t in traders:
        ...
        result.append({
            ...
            "total_return_pct": t.total_return_pct if t.total_return_pct else None,
            "risk_score": t.risk_score if t.risk_score else None,
            "risk_classification": t.risk_classification or "unknown",
            "max_drawdown": t.max_drawdown if t.max_drawdown else None,
            "volatility": t.volatility if t.volatility else None,
            "avg_monthly_return": t.avg_monthly_return if t.avg_monthly_return else None,
            "sharpe_score": t.sharpe_score if t.sharpe_score else None,
...
async def discover_top_traders(
    ...
    try:
        result = await client.enrich_candidates(unique, max_concurrent=8)
    except Exception as e:
